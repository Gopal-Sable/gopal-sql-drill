...
-- channel data
INSERT INTO channels(id, channel_name, organization_id) VALUES 
(1, "general", 1),
(2, "random", 1);
...
INSERT INTO messages(message, user_id, channel_id) VALUES 
...
SELECT user_name, COUNT(message) AS message_count 
FROM messages m 
JOIN users u ON u.id = m.user_id
GROUP BY user_name;

-- [Stretch!] List the count of messages per user per channel.
SELECT user_name, channel_name, COUNT(message) AS message_count 
FROM messages m 
JOIN channels c ON m.channel_id = c.id
JOIN users u ON u.id = m.user_id
GROUP BY user_name, channel_name;
